...
<table class="table table-sm text-right mt-5" style="width: 100%; margin-bottom: 30px;">
  <tbody>
  	<tr class="table-success">
		<th colspan="5" style=" text-align: center; font-size: 130%">Pagos nos últimos <?php echo $months?> meses (ordem descrescente)</th>
  	</tr>
  	<?php foreach($repasses as $data=>$i){
  	    $total_data = 0;
  	    ?>
      	<tr class="table-success">
      		<th colspan="5" style="text-align: left; border-bottom: 1px solid black;">Pago em <?php echo $data?></th>
      	</tr>
      	<?php foreach($i as $r){
      	    $total_data+=$r['valor'];
          	?>
        ...
        <tr class="table-secondary" style="border-top: 1px solid black">
      		<th colspan="4" style=" text-align: right;">Total pago em <?php echo $data?></th>

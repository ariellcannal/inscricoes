...
// debug enabled, great for debug and development, you can remove this two lines
// for production app
error_reporting(E_ALL); // error reporting (debug)
ini_set('display_errors', ini_get('display_errors')); // error showing (debug)

// direct access to DB driver and config
define('XCRUD_PATH', str_replace('\\', '/', dirname(__file__))); // str_replace
                                                                 // - windows
                                                                 // trick
require (XCRUD_PATH . '../../../config/xcrud.php'); // configuration
require (XCRUD_PATH . '/xcrud_db.php'); // database class
mb_internal_encoding(Xcrud_config::$mbencoding); // sets multibyte encoding
                                                 // globaly
...
class Xcrud
{
    ...
    protected $search_lines = 1;

    protected $disabled_conditions = array();
    ...
    protected function __construct()
    {
        ...
        $this->date_format = array(
            ...
            'php_y' => Xcrud_config::$php_year_format,
            'moment_d' => Xcrud_config::$moment_date_format,
            'moment_t' => Xcrud_config::$moment_time_format,
            'moment_y' => Xcrud_config::$moment_year_format
        );
        $this->nested_readonly_on_view = Xcrud_config::$nested_readonly_on_view;
    }
    ...
    public static function get_requested_instance()
    {
        ...
        // if (isset($_SESSION['lists']['xcrud_session'][$inst_name]['key']) && $_SESSION['lists']['xcrud_session'][$inst_name]['key'] == $key) {
        if (1 == 1) {
    ...
    protected static function init_prepare($method = false)
    {
        switch ($method) {
            case 'post':
                $sess_name = (Xcrud_config::$dynamic_session && isset($_POST['xcrud']['sess_name']) && $_POST['xcrud']['sess_name']) ? $_POST['xcrud']['sess_name'] : Xcrud_config::$sess_name;
                break;
            case 'get':
                $sess_name = (Xcrud_config::$dynamic_session && isset($_GET['xcrud']['sess_name']) && $_GET['xcrud']['sess_name']) ? $_GET['xcrud']['sess_name'] : Xcrud_config::$sess_name;
                break;
            default:
        ...
        if ($instance_name && $field && $inner_tbl && $tbl_field) {
            ...
            foreach ($fdata as $fitem) {
                $this->inner_table_instance[$instance_name] = $fitem['table'] . '.' . $fitem['field']; // name
                                                                                                       ...
                                                                                                       // parent
                                                                                                       // instance
                $instance = Xcrud::get_instance($instance_name); // just another
                                                                 // xcrud object
    ...
    public function render_columns_select()
    {
        if (! $this->columns_select)
            return null;
        $out = "";
        $out .= $this->open_tag('select', 'selectbox xcrud-columnsList-select', array(
        ...
        foreach ($this->fields_list_default as $field => $fdata) {
            ...
            $attr['value'] = $field;
            $out .= $this->open_tag('option', null, $attr) . $this->html_safe($this->columns_names[$field]) . $this->close_tag('option');
        }
     ...
     */
    public function button($link = '', $name = '', $icon = '', $class = '', $parameters = array(), $conditions = array(), $modes = null)
    {
        if ($link) {
            ...
            foreach ($fdata as $fitem) {
                ...
                switch ($type) {
                    ...
                    case 'image':
                        ...
                        break;
                    case 'price':
                        $this->field_attr[$fitem['table'] . '.' . $fitem['field']] = array_merge((array) $attr);
                        break;
                    case 'select':
                    case 'multiselect':
                    case 'radio':
    ...
    public function disabled($fields = '', $mode = false, $cond_field = null, $cond_operator = null, $cond_value = null)
    {
        if ($cond_field && ! is_array($cond_field)) {
            if (! strpos($cond_field, '.')) {
                $cond_field = $this->table . "." . $cond_field;
            }
            $conditions[0] = $cond_field;
            $conditions[1] = $cond_operator;
            $conditions[2] = $cond_value;
        } else {
            $conditions = null;
        }
        ...
        foreach ($fdata as $key => $fitem) {
            if ($conditions) {
                $this->disabled_conditions[$key] = $conditions;
            } else {
     ...
     *         COMPATIBILIZAÇÃO COM A ARQUITETURA DO CODEIGINITER
     */
    public function after_clone($callable = '', $path = 'functions.php')
    {
        if ($callable && $path) {
            $this->after_clone['callable'] = $callable;
            $this->after_clone['path'] = "/../../helpers/" . $path;
        }
        return $this;
    }
    ...
    protected function render_custom_csv()
    {
        ...
        while ($row = $db->result->fetch_assoc()) // low level result process,
                                                   // saves memory
        {
            $out = array();
    ...
    public function _csv()
    {
        ...
        if ($db->result->num_rows > Xcrud_config::$csv_limit)
    ...
    protected function prepare_query_field($val, $key, $action, $no_processing = false)
    {
        ...
        if ($no_processing) {
            ...
            if (is_array($val)) {
                ...
                $val = (float) str_replace($this->field_attr[$key]['point'], '.', $val);
    ...
    protected function _remove()
    {
        ...
        if ($this->replace_remove) {
            ...
            if (is_callable($this->replace_remove['callable'])) {
                $del = call_user_func_array($this->replace_remove['callable'], array(
                    $this->primary_val,
                    $this
                ));
            }
            ...
            // images && fk
            if (in_array('image', $this->field_type) or in_array('file', $this->field_type) or in_array('fk_relation', $this->field_type)) {
    ...
    protected function _build_where($build_alphabetical = true, $is_totalizer = false)
    {
        ...
        if (! in_array($this->task, array(
            ...
            foreach ($this->custom_filter as $filter_name => $opts) {
                unset($current_label);
                $opts = $opts['options'];
                if (isset($this->custom_filter[$filter_name]['active'])) {
                    $current_label = $this->custom_filter[$filter_name]['active'];
                    $custom_filter = $this->custom_filter[$filter_name]['options'][$current_label];
    ...
    protected function _receive_post($task = false, $primary = false)
    {
        ...
        if ($this->is_get) {
            ...
            if (isset($_SESSION['lists']) && isset($_SESSION['lists']['xcrud_session'][$this->instance_name]))
                $this->alphabetical_filter = $this->_post('alphabetical', $_SESSION['lists']['xcrud_session'][$this->instance_name]['alphabetical_filter']);
            ...
            if ($filter && $filter_label) {
                switch ($filter_label) {
                    case "all":
                        unset($this->custom_filter[$filter]['active']);
                        break;
                    default:
                        $this->custom_filter[$filter]['active'] = $filter_label;
                }
            }
        }
    }
    ...
    protected function _define_field_type($row, $field_index)
    {
        ...
        switch ($type) {
            ...
            case 'decimal':
                ...
                if (! isset($this->field_type[$field_index])) {
                    $this->field_type[$field_index] = 'price';
                }

                if ($this->field_type[$field_index] == 'price') {
                    if (! isset($this->field_attr[$field_index]['separator'])) {
                        $this->field_attr[$field_index]['separator'] = ',';
                    }

                    if (! isset($this->field_attr[$field_index]['point'])) {
                        $this->field_attr[$field_index]['point'] = ',';
                    }

                    $this->field_attr[$field_index]['decimals'] = explode(',', $matches[2])[1];
                }
                $debug = $this->field_attr[$field_index];
    ...
    protected function _render_list()
    {
        ...
        $mode = 'list';
        $view_file = Xcrud_config::$themes_path . '/' . $this->theme . '/' . $this->load_view['list'];
                                                                                  ...
                                                                                  // objects
        {
            foreach ($this->inner_table_instance as $inst_name => $field) {
                if (isset($this->result_row[$field])) {
                    ...
                    $this->nested_rendered[$inst_name] = '<div class="xcrud-nested-container xcrud-container"><div class="xcrud-ajax" id="xcrud-ajax-' . base_convert(rand(), 10, 36) . '">' . $instance->render($instance->nested_default_render, $instance->nested_default_render_primary) . '</div></div>';
                }
            }
        }

        $view_file = Xcrud_config::$themes_path . '/' . $this->theme . '/' . $this->load_view[$mode];
        $view_file = $this->check_file($view_file, 'render');
        ob_start();
        include ($view_file);
    ...
    public function _export_vars()
    {
        ...
        if ($this->instance_count == 1 && ! $this->ajax_request) {
            if (isset($_SESSION['lists']['xcrud_session']) && $_SESSION['lists']['xcrud_session']) {
                foreach ($_SESSION['lists']['xcrud_session'] as $s_key => $s_val) { // workaround
                                                                                    // on
                                                                                    // some
                                                                                    // servers
                                                                                    // session
    ...
    public function import_vars($key = false)
    {
        ...
        foreach ($this->params2save() as $item) {
             ...
             *
             * @author Ariel Canal
             *         O original cont�m um erro de
             *         l�gica. Caso o atributo desejado n�o esteja
             *         armazenado
             *         na sess�o, ele atribui NULL no atributo da classe,
             *         fazendo com que a inst�ncia n�o funcione
             *         e apresente o erro: Incorrect table name '' SHOW COLUMNS
             *         FROM ``
             *         Corre��o: S� sobrescreve o atributo inicial pelo
             *         da
             *         sess�o, caso este exista, na sess�o.
             */
    ...
    protected function get_field_attr($name, $mode)
    {
        ...
        if (isset($this->disabled_conditions[$name]) && isset($this->result_row[$this->disabled_conditions[$name][0]]) && $mode == 'edit') {
            $f = $this->disabled_conditions[$name][0];
            $o = $this->disabled_conditions[$name][1];
            $v = $this->disabled_conditions[$name][2];
            if (! strpos($f, '.')) {
                $f = $this->table . "." . $f;
            }
            // var_dump($this->_compare($this->result_row[$f], $o, $v));
    ...
    protected function create_price($name, $value = '', $tag = array())
    {
        ...
        if (! empty($this->field_attr[$name]['prefix'])) {
            $mask = $this->html_safe($this->field_attr[$name]['prefix']);
        } else {
            $mask = '';
        }

        if (! empty($this->field_attr[$name]['maxlength'])) {
            $length = $this->field_attr[$name]['maxlength'] - ($this->field_attr[$name]['decimals'] + 1);
        } else {
            $length = 10;
        }

        $thousands = [];
        for ($i = 1; $i <= floor($length / 3); $i ++) {
            $thousands[] = '###';
        }
        $done_extra_digits = false;
        for ($i = 1; $i <= ($length - floor($length / 3) * 3); $i ++) {
            $mask .= '#';
            $done_extra_digits = true;
        }
        if ($done_extra_digits) {
            $mask .= $this->field_attr[$name]['separator'];
        }
        $mask .= implode($this->field_attr[$name]['separator'], $thousands);
        $mask = substr_replace($mask, "0", - 1);
        $mask .= $this->field_attr[$name]['point'];

        for ($i = 1; $i <= $this->field_attr[$name]['decimals']; $i ++) {
            $mask .= '0';
        }

        if (! empty($this->field_attr[$name]['suffix'])) {
            $mask .= $this->html_safe($this->field_attr[$name]['suffix']);
        }
    ...
    protected function create_moment_masks($type)
    {
        if (! function_exists('create_mask')) {

            function create_mask($format, $separador = '')
            {
                $mask = '';
                if (! empty($separador)) {
                    $mask_a = explode($separador, $format);
                } else {
                    $mask_a = [
                        $format
                    ];
                }
                foreach ($mask_a as $k => $part) {
        ...
        switch ($type) {
            case 'date':
                return create_mask($this->date_format['moment_d'], '/');
            case 'time':
                return create_mask($this->date_format['moment_t'], ':');
            case 'year':
                return create_mask($this->date_format['moment_y']);
            case 'datetime':
                return create_mask($this->date_format['moment_d'], '/') . ' ' . create_mask($this->date_format['moment_t'], ':');
        }
    }
    ...
    protected function create_relation($name, $value = '', $tag = array(), $dependval = false)
    {
        ...
        if ($total['total'] >= Xcrud_config::$relation_ajax) {
        ...
        $out .= $this->open_tag($tag, $this->theme_config('relation_field'), $this->field_attr[$name]);
    ...
    public function create_file($name, $value = '', $tag = array(), $is_upload = false)
    {
        if (isset($this->subselect[$name])) {
    ...
    public function create_image($name, $value = '', $tag = array(), $is_upload = false)
    {
        if (isset($this->subselect[$name])) {
    ...
    protected function create_remote_image($name, $value = '', $tag = array())
    {
        ...
        unset($attr['link']);
        $out .= $this->single_tag($tag, $this->theme_config('remote_image_field'), $attr);
        return $out;
    }
    ...
    protected function create_view_point($name, $value = '', $tag = array())
    {
        ...
        if ($value) {
            ...
            $map = array(
                ...
                'class' => 'xcrud-map',
                'style' => 'width:' . $attr['width'] . 'px;height:' . $attr['height'] . 'px;',
                'src' => 'https://maps.googleapis.com/maps/api/staticmap?center=' . $value . '&zoom=' . $attr['zoom'] . '&size=' . '2000x' . $attr['height'] . '&maptype=roadmap&markers=color:red%7C' . $value . '&key=' . Xcrud_config::$maps_api_key
            );
            unset($attr['text'], $attr['zoom'], $attr['width'], $attr['height'], $attr['search_text']);
            $out .= $this->single_tag($tag, $this->theme_config('point_field'), $attr);
    ...
    protected function _upload_image()
    {
        ...
        if (isset($_FILES) && isset($_FILES['xcrud-attach']) && ! $_FILES['xcrud-attach']['error']) {
            ...
            if (! is_null($oldfile) && $oldfile != $filename) {
                $this->upload_to_remove[$oldfile] = $field;
            }
            $this->upload_to_save[$filename] = $field;
    ...
    protected function _render_list_buttons(&$row)
    {
        ...
        if ($this->buttons) {
            foreach ($this->buttons as $button) {
                if ($button['modes'] != "" && ! in_array($this->task, explode(',', $button['modes']))) {
                    continue;
                }
    ...
    {
        if (is_file(XCRUD_PATH . '/' . Xcrud_config::$lang_path . '/' . $this->language . '/xcrud.ini'))
            self::$lang_arr = parse_ini_file(XCRUD_PATH . '/' . Xcrud_config::$lang_path . '/' . $this->language . '/xcrud.ini');
        elseif (is_file(XCRUD_PATH . '/' . Xcrud_config::$lang_path . '/en/xcrud.ini'))
            self::$lang_arr = parse_ini_file(XCRUD_PATH . '/' . Xcrud_config::$lang_path . '/en/xcrud.ini');
        if ($this->set_lang) {
     ...
     *         COMPATIBILIZAÇÃO COM A ARQUITETURA DO CODEIGINITER
     */
    public static function _get_language_static()
    {
        if (is_file(XCRUD_PATH . '/' . Xcrud_config::$lang_path . '/' . Xcrud_config::$language . '/xcrud.ini'))
            self::$lang_arr = parse_ini_file(XCRUD_PATH . '/' . Xcrud_config::$lang_path . '/' . Xcrud_config::$language . '/xcrud.ini');
     ...
     *         COMPATIBILIZAÇÃO COM A ARQUITETURA DO CODEIGINITER
     */
    public function _get_theme_config()
    { // loads theme configuration from
      // ini file
        if (is_file(XCRUD_PATH . '/' . Xcrud_config::$themes_path . '/xcrud_default/xcrud.ini'))
            $this->theme_config = parse_ini_file(XCRUD_PATH . '/' . Xcrud_config::$themes_path . '/xcrud_default/xcrud.ini');
        else
    ...
    protected function lang($text = '')
    {
        ...
        return htmlspecialchars((isset(self::$lang_arr[$langtext]) ? self::$lang_arr[$langtext] : $text), ENT_QUOTES, Xcrud_config::$mbencoding);
    }
    ...
    public static function load_css()
    {
        ...
        if (Xcrud_config::$load_bootstrap) {
            $out .= '<link href="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri . '/bootstrap/css/bootstrap.min.css?' . time() . '" rel="stylesheet" type="text/css" />';
        }
        if (Xcrud_config::$load_jquery_ui)
            $out .= '<link href="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri . '/jquery-ui/jquery-ui.min.css?' . time() . '" rel="stylesheet" type="text/css" />';
        if (Xcrud_config::$load_jcrop)
            $out .= '<link href="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri . '/jcrop/jquery.Jcrop.min.css?' . time() . '" rel="stylesheet" type="text/css" />';
    ...
    public static function load_js()
    {
        ...
        if (Xcrud_config::$load_jquery_ui)
            $out .= '<script src="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri . '/jquery-ui/jquery-ui.min.js?v=' . time() . '"></script>';
        if (Xcrud_config::$load_jcrop) {
            $out .= '<script src="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri . '/jcrop/jquery.Jcrop.min.js?v=' . time() . '"></script>';
        }
        if (Xcrud_config::$editor_url)
            $out .= '<script src="' . Xcrud_config::$editor_url . '?v=' . time() . '"></script>';
        if (Xcrud_config::$load_googlemap)
            $out .= '<script src="//maps.google.com/maps/api/js?sensor=false&language=' . $language . '&v=' . time() . '"></script>';
        $out .= '<script src="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$js_uri . '/xcrud.js?v=' . time() . '"></script>';

        $out .= '<script type="text/javascript">CKEDITOR.timestamp = "' . rand(0, 999) . '";</script>';
        ...
        $config = array(
            ...
            'moment_date_format' => Xcrud_config::$moment_date_format,
            'moment_time_format' => Xcrud_config::$moment_time_format,
            'moment_year_format' => Xcrud_config::$moment_year_format,
            'lang' => self::$lang_arr,
    ...
    protected function find_details_text_variables()
    {
        ...
        /*
         * ALTERADO DO PADRAO - Compatibilizacao com array de
         * grid_restrictions e disabled_conditions
         */
        if ($this->grid_restrictions) {
        ...
        if ($this->disabled_conditions) {
            foreach ($this->disabled_conditions as $k => $v) {
    ...
    protected function open_tag($tag = '', $class = '', $attr = array())
    {
        if ($tag) {
            ...
            if (isset($tag['tag'])) {
                ...
                if (Xcrud_config::$encode_field_names && isset($tag['data-depend'])) {
                    $tag['data-depend'] = $this->fieldname_encode($tag['data-depend']);
                }
                if (Xcrud_config::$encode_field_names && isset($tag['data-rangestart'])) {
                    $tag['data-rangestart'] = $this->fieldname_encode($tag['data-rangestart']);
                }
                if (Xcrud_config::$encode_field_names && isset($tag['data-rangeend'])) {
    ...
    protected function open_label_tag($field_key, $label_tag = 'td', $extra_class = '')
    {
        if (isset($this->validation_required[$this->fields_output[$field_key]['name']])) {
            $label_class = $this->theme_config('details_label_cell') . ' ' . $extra_class . ' xcrud-label-required';
        } else {
            $label_class = $this->theme_config('details_label_cell') . ' ' . $extra_class;
        }
        $attr = [];
    ...
    protected function render_fields_list($mode, $container = 'table', $row = 'tr', $label = 'td', $field = 'td', $tabs_block = 'div', $tabs_head = 'ul', $tabs_row = 'li', $tabs_link = 'a', $tabs_content = 'div', $tabs_pane = 'div')
    {
        ...
        if (isset($this->field_tabs[$mode]) or $this->default_tab !== false) {
            ...
            $tab_count = 0;
            if ((isset($this->field_tabs[$mode]) && $tabs_out) || ($this->nested_rendered && Xcrud_config::$nested_in_tab)) {
    ...
    protected function render_tab($tab, $mode, $container = 'table', $row = 'tr', $label = 'td', $field = 'td', $tabs_block = 'div', $tabs_head = 'ul', $tabs_row = 'li', $tabs_link = 'a', $tabs_content = 'div', $tabs_pane = 'div')
    {
        ...
        if ((isset($this->field_tabs[$mode]) or $this->default_tab !== false) && ! Xcrud_config::$tabs_in_widgets) {
            $tabs_header = $this->open_tag($tabs_block, $this->theme_config('tabs_container'), array(
                'class' => 'xcrud-tabs'
            )) . $this->open_tag($tabs_head, $this->theme_config('tabs_header_row'));
            $tabs_body = $this->open_tag($tabs_content, $this->theme_config('tabs_content'));
            ...
            if (isset($this->field_tabs[$mode]) && $tabs_out) {
                foreach ($this->field_tabs[$mode] as $key => $tabname) {
                    if ($key == $tab) {
                        ...
                        if (Xcrud_config::$widgets_open != 'all' && Xcrud_config::$widgets_open != 'none' && $k >= (int) Xcrud_config::$widgets_open)
                            $class_widget_container .= ' widget-closed';
                        else if (Xcrud_config::$widgets_open == 'none')
                            $class_widget_container .= ' widget-closed';
                        ...
                        $widget .= $this->open_tag('i', 'fa fa-reorder') . $this->close_tag('i');
                        $widget .= $tabname;
                        ...
                        $widget .= $this->open_tag('i', 'fa fa-angle-down') . $this->close_tag('i');
     ...
     */
    protected function render_button($name = '', $task = '', $after = '', $class = '', $icon = '', $mode = '', $primary = '', $attr = [])
    {
        $out = '';
    ...
    protected function safe_file_name($file, $field)
    {
        ...
        if (isset($this->upload_config[$field]['not_rename']) && $this->upload_config[$field]['not_rename'] == true)
            ...
            $filename = base_convert(str_replace(' ', '', time()) . rand(), 10, 36) . $ext;
        return $filename;
    }
    ...
    {
        $file_path = $this->get_image_folder($field) . '/' . $filename;
        $file_path = mb_convert_encoding($file_path, "Windows-1252", "UTF-8");
        move_uploaded_file($file['tmp_name'], $file_path);
        if ($this->after_upload) {
    ...
    protected function is_edit(&$row)
    {
        if (! isset($this->grid_restrictions['edit'])) {
            ...
            if (($item['bool'] === true && in_array(true, $cond)) || ($item['bool'] === false && ! in_array(true, $cond))) {
                $this->is_edit = false;
                return false;
            }
    ...
    /*
     * ALTERADO DO PADR�O - Compatibilização com array de grid_restrictions
     */
    ...
    {
        if (! Xcrud_config::$encode_field_names) {
            return $name;
        }
     ...
     */
    public function cast_number_format($number, $field, $edit = false)
    {
        $out = '';
        ...
        if (! empty($this->field_attr[$field]['prefix'])) {
        ...
        if (! empty($this->field_attr[$field]['suffix'])) {
    ...
    public function custom_filter($filter_name, $filter_opts, $default = false, $container_class = null, $toggle_class = null)
    {
        if (! array_key_exists('all', $filter_opts)) {
        ...
        $this->custom_filter[$filter_name]['options'] = $filter_opts;
        $this->custom_filter[$filter_name]['container_class'] = $container_class;
        $this->custom_filter[$filter_name]['toggle_class'] = $toggle_class;
        $this->custom_filter[$filter_name]['active'] = ($default) ? $default : 'all';
    }
    ...
    public function render_totalizers($render_view_all = true, $display = 'block')
    {
        if ($total_items = count($this->totalizers)) {
            ...
            if ($render_view_all) {
                ...
                $out .= '<a class="xcrud-action more" data-filter="totalizer" data-label="all">Visualizar <i class="pull-right fa fa-angle-right"></i></a>';
                $out .= "</div></div></div>";
            }
    ...
    private function apply_record_changes($set)
    {
        if (is_array($this->record_changes) && sizeof($this->record_changes)) {
            ...
            if (sizeof($changes)) {
                $ci = &get_instance();
                $ci->usuarios_model->registraAlteracao($this->table, $this->primary_val, implode(PHP_EOL . PHP_EOL, $changes));
            }
        }
    }
    ...
    protected function relation_search($name, $dependval)
    {
        ...
        $db->query('SELECT `' . $this->relation[$name]['rel_field'] . '` AS `field`,' . $name_select . $this->get_relation_tree_fields($this->relation[$name]) . ' FROM `' . $this->relation[$name]['rel_tbl'] . '` ' . $where . ' GROUP BY `field` ORDER BY ' . $this->get_relation_ordering($this->relation[$name]) . ' LIMIT ' . Xcrud_config::$relation_ajax);
        $options = $this->resort_relation_opts($db->result(), $this->relation[$name]);
    ...
    public function custom_lists($static = false, $default = false)
    {
        ...
        $this->custom_filter['title']['active'] = $default;
        return $this;
    }
    ...
    private function set_custom_lists()
    {
        ...
        if (in_array($this->custom_filter['title']['active'], array_keys($this->custom_lists_static))) {
            $this->columns($this->columns_default);
        }
        foreach ($db->result() as $list) {
            if ($this->custom_filter['title']['active'] == $list['lpe_nome']) {
        ...
        $this->custom_filter('title', $filtro_titulo, $this->custom_filter['title']['active']);
    }
    ...
    public function render_title($mode = 'list', $tag = 'h2', $to_show = false, $icon = false, $replace_title = false)
    {
        ...
        if ($this->is_title) {
            ...
            if ($this->custom_lists) {
                ...
                $out .= $this->close_tag('i');
                $out .= "&nbsp;" . $this->table_name . "&nbsp;";
                if ($this->custom_filter['title']['active'] != "") {
                    $out .= $this->open_tag('small');
                    $out .= $this->custom_filter['title']['active'] != 'all' ? $this->custom_filter['title']['active'] : "";
                    $out .= $this->close_tag('small') . "&nbsp;";
                }
                $out .= $this->open_tag('i', 'fa fa-chevron-down');
                $out .= $this->close_tag('i');
                $out .= $this->close_tag('span');
                $out .= $this->close_tag($tag);
                $out .= $this->close_tag('a');
                ...
                if ($this->custom_lists_static) {
                    foreach ($this->custom_lists_static as $label => $where) {
                        ...
                        $out .= $this->open_tag('i', 'fa fa-list-alt fa-fw');
                        $out .= $this->close_tag('i');
                ...
                foreach ($this->custom_lists_temp as $label => $where) {
                    ...
                    $out .= $this->open_tag('i', 'fa fa-filter fa-fw');
                    $out .= $this->close_tag('i');
                    $out .= $label . $this->close_tag('a');

                    if ($label == $this->custom_filter['title']['active']) {
                        $out .= $this->open_tag('i', 'btn btn-xs btn-default fa fa-pencil edit', array(
                            'data-toggle' => 'modal',
                ...
                $this->custom_filter('title', $filtro_titulo, ($this->custom_filter['title']['active'] != "") ? $this->custom_filter['title']['active'] : $default_list, true);
                ...
                $out .= $this->open_tag('i', 'fa fa-plus fa-fw');
                $out .= $this->close_tag('i');
    ...
    {
        $out = "";
        if (isset($this->mass_actions['edit']) && count($this->mass_actions['edit'])) {
            $out .= $this->open_tag('div', 'xcrud-mass-form ' . $this->theme_config('mass_form_container'));
            foreach ($this->mass_actions['edit'] as $field => $fitem) {
                $type = $this->field_type[$field];
                ...
                if (method_exists($this, $func))
                    $field = call_user_func_array(array(
                        ...
                        null,
                        [
                            'name' => $this->fieldname_encode($field),
    ...
    {
        $fields = $this->_parse_field_names($field, 'mask');
        foreach ($fields as $key => $field) {
            $this->field_attr[$key]['data-mask'] = $mask;
        }
    }
}

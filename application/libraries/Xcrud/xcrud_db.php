...
class Xcrud_db
{
    ...
    public function query($query = '')
    {
        ...
        // echo '<pre>' . $query . '</pre>';
        if ($this->connect->error) {
            $this->error($this->connect->error . '<pre>' . $query . '</pre>', $this->connect->errno, $this->connect->error);
        }
    ...
    ))
    {
        ...
        if ($val == '') {
            ...
            return '\'' . $pattern[0] . ($this->magic_quotes ? (string) str_replace(' ', '%', $val) : $this->connect->real_escape_string((string) str_replace(' ', '%', $val))) . $pattern[1] . '\'';
        }
    }
